...
* テーマは WordPress.org のプラグインディレクトリにあるプラグインを推奨することのみできます。

* テーマ内においてはプラグインを推薦する目的で [TGM Plugin Activation](http://tgmpluginactivation.com/) を使うことができます。

* テーマ内にオプションフレームワークのようなライブラリ（ただし要求事項を見対している必要があります）を含むこともできます。

## スクリーンショット - Screenshot

* スクリーンショットがテーマの外観がどう見えるのかを適切に表現しているものであること。

* スクリーンショット内ではサポートされているプラグインや設定、テンプレートを表示させることも可能です。
